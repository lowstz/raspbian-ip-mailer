...
# Settings
SCRIPT_FILE=server-ip-mailer.py
SCRIPT_PATH=/usr/local/bin
LAUNCHER_FILE=server-ip-mailer
LAUNCHER_PATH=/etc/init.d

log_action_msg Installing [ server-ip-mailer ] \( by zshenjia@gmail.com \) ...

# Download script
log_begin_msg Downloading [ $SCRIPT_FILE ] from Github to [ $SCRIPT_PATH ]...
sudo wget https://raw.github.com/lowstz/server-ip-mailer/master/$SCRIPT_FILE -O $SCRIPT_PATH/$SCRIPT_FILE -o /dev/null
log_end_msg $?

# Download laucher
log_begin_msg Downloading [ $LAUNCHER_FILE ] from Github to [ $LAUNCHER_PATH ]...
sudo wget https://raw.github.com/lowstz/server-ip-mailer/master/$LAUNCHER_FILE -O $LAUNCHER_PATH/$LAUNCHER_FILE  -o /dev/null
log_end_msg $?
